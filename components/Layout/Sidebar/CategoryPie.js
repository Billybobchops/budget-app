...
const CategoryPie = () => {
	...
	const ResponsivePie = dynamic(() => import('@nivo/pie').then(module => module.ResponsivePie), { ssr: false });
	...
	const getPercent = (categories, income, items) => {
		...
		Object.values(items).map((item) => {
			tempArr.map((category, i) => {
				if (category.id === item.category) {
					...
					tempArr[i].value = +(tempArr[i].budgetedItemsTotal / totalPay).toFixed(2);
				}
			});
		...
		let categorized = 0;
		tempArr.map((category) => { categorized = categorized + category.value });
	...
	return (
		<div className={classes.container}>
			...
			{income && totalIncome > totalBudget && (<CategoryPieCards categories={data} colors={colors} />)}
		</div>
	);
};
